...
importing NifFormat::
    ...
    import asset_convert.nif.pyffi_monkey_patch  # apply patches
        ...
        class BSTriShape(NifFormat.NiAVObject):
            ...
            def write(self, stream, data=None):
                raise NifFormat.NifError(
                    ...
                    'asset_convert.nif.sse_nif.sse_to_le() before writing')
...
def apply_patches():
    ...
    try:
        ...
        try:
            from asset_convert.nif import nif_geom_native
            nif_geom_native.install()
        except Exception:
            pass
        # Patch 14: numpy-backed Array storage (asset_convert/nif/nif_geom_array.py).
        # Supersedes Patch 13's fill/pack for the element types it backs; the
        # native hook above still serves anything it declines.
        try:
            from asset_convert.nif import nif_geom_array

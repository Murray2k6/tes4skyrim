...
def capture_string_masks(data) -> dict:
    ...
    for root in data.roots:
        ...
        for block in root.tree():
            ...
            if mask.any():
                masks[get_block_name(block)] = mask
    return masks
...
def _has_bow_rig(root) -> bool:
    for block in root.tree():
        if isinstance(block, NifFormat.NiNode) and \
                get_block_name(block) == 'Bow_MidBone':
            return True
    return False
...
def add_bow_rig(data, string_masks: dict) -> int:
    ...
    for root in data.roots:
        ...
        for b in shapes:
            ...
            G = m44_to_np(b.get_transform(root))
            vr = vs @ G[:3, :3] + G[3, :3]
        ...
        for block in shapes:
            ...
            G = m44_to_np(block.get_transform(root))
            vr = vs @ G[:3, :3] + G[3, :3]   # verts in root/attach frame

            mask = string_masks.get(get_block_name(block))
            ...
            manual_update_bind_position(block, skin, root)
            for slot, bi in enumerate(used):
                entry = skin_data.bone_list[slot]

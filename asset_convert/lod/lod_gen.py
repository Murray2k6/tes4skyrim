...
FLAG_DISTANT_LOD  = 0x00008000   # Has Distant LOD — SSELodGen bakes LOD for this object
FLAG_WORLD_MAP    = 0x10000000   # Show in World Map — object appears on the world map
...
def _parse_esm(esm_path: Path):
    ...
    `formid_remap_table`). Raw ids cannot be compared across plugins — the
    ...
    def _dispatch(rec, pw, pc):
        ...
        if sig == 'WRLD':
            ...
            if data_sub and len(data_sub) >= 24:
                x, y, z, rx, ry, rz = (
                    finite(v) for v in struct.unpack_from('<6f', data_sub))
            scale = 1.0
            xscl = _sub(subs, 'XSCL')

...
- **`SetNumericGameSetting` targets SKSE64** — the converter emits the typed
  `Game.SetGameSettingFloat/Int/Bool` native and compiles it through the
  temporary SKSE64 header overlay. The matching vanilla getter reads the same
  global setting, preserving the save/restore idiom. Settings absent from
  Skyrim keep a `;TODO` marker rather than a call that silently does nothing.
...
delivers bound gear through Abilities (`SPIT.Type` 4), which Skyrim applies
passively. The Mythic Dawn assassins in the Imperial Dungeon wear
`AbBoundArmorMaceNoHelmetMD`, an Ability.

**Lesser powers are castable.** The earlier text incorrectly called them passive.
The 2026-09-05 Skyrim.esm census found all three Type-3 spells use CastType 1
(Fire-and-Forget), including `VampireHuntersSight`. All 13 diseases use CastType 0;
abilities include 247 CastType-0 and three CastType-1 records. xEdit's SPIT enum
and CommonLib's `MagicSystem::SpellType` agree: 1 = Disease, 3 = Lesser Power,
4 = Ability. Probe: `temp/vanilla_spell_type_probe.py`; measured results:
`temp/vanilla_spell_type_probe.log`. The diagnostic ESM reader also had incorrect
type labels and offsets for the trailing SPIT fields; those are corrected.

So the routing rule keys on **both** the item type and the delivery:

| | Castable spell / power (Types 0, 2, 3) | Ability / Disease (Types 4, 1) |
|---|---|---|
...
- **Skyrim has vanilla Papyrus GMST readers; writers are SKSE64.** The script
  compile phase augments headers only for type checking and ships no replacement
  base scripts.

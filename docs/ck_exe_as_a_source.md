...
## Verified local executable availability (2026-09-05)

The Steam-path executable at
`E:/SteamLibrary/steamapps/common/Skyrim Special Edition/SkyrimSE.exe`
(SHA256 `846EFCCF0C1374D71F892907F46549560F2FCB0A75CB87A3EED438BAA0F1402F`)
has readable engine code on disk. Its `.bind` section does not imply its
`.text` is encrypted. Check the actual code before requiring a different copy.
`oblivion_exe_probe.py` supports this 64-bit PE; fast loading avoids parsing
unneeded import/resource directories.

Verified command handlers: `SetCombatStyle` RVA `359B30` calls actor setter
`6C8060`, which updates extra data, adds change flag `80000000`, and refreshes
the combat controller. `SendTrespassAlarm` RVA `35FA90` calls the criminal's
`TrespassAlarm(witness, witness base, -1)`. `GetTalkedToPC` condition RVA
`332000` reads the reference's talked-to flag. `TrapUpdate` RVA `35CF90`
is a stub (`mov al,1; ret`), so invoking that command cannot reproduce traps.
These conclusions come from the game executable. The corresponding legacy
command callbacks inspected in the CK were editor stubs.

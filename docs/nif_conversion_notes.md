...
## Flame sockets inherit source masters (2026-09-05)

Morroblivion's `lights/torchnohavok.nif` and `lights/dunmer/candle_red_01.nif`
contain empty FlameNode1/0 sockets but no own FlameNode STAT records. The old
reader returned an empty map and reported both conversions successful without
attaching their flames. The map now reads transitive masters before child
records, including grouped record directories and asset-only `.base_plugins`.
Model resolution preserves the complete authored MODL path and honors a mod's
replacement file at that path. Missing/corrupt named flame models fail conversion.
The existing zero-padded-socket behavior and marker transforms are unchanged.

Ten targeted tests pass. The real scoped rebuild converted 151 meshes across
Frostcrag, Maskar, Midas, and both Morroblivion plugins; eight affected Midas
creature skeletons were also rebuilt in their existing project directories.
Readback verified populated candle/torch sockets and their flipbook textures,
and Frostcrag's FlameNode01 remained empty. This fixes inherited attachments;
the 20 Maskar morph-emitter write failures and script-driven Add/RemoveFlames
remain unresolved. No in-game verification is claimed.

## Metadata is produced by asset stages (2026-09-05)

Mesh builds now refresh bounds and collision together after the mesh, hair,
grass, texture and generated book-art work. SpeedTree and creature stages also
refresh their generated NIFs, so later asset stages cannot leave an earlier
cache stale. Import retains a combined scan only for missing/outdated caches.
With current caches, the refresh parses only rewritten outputs and removes
deleted entries; untouched entries survive. Each selected NIF is parsed once
for both analyses. Metadata parse failures are reported and leave both old
caches untouched instead of silently dropping the model's geometry.

An ephemeral before/after file-state inventory selects writes; its timestamps
are never stored in a cache key. Windows directory-entry stats reduced this
inventory from 10.366 to 0.330 seconds over 14,322 Nehrim NIFs. Already-Skyrim
copies preserve their bytes but use their actual output write time, so a copied
replacement participates in the refresh. Binary collision serialization handles
both scanner lists and loaded NumPy arrays without converting every element
back through Python's argument stack.

The real scoped build rebuilt two paintbrushes under each of Oblivion and
Nehrim, then scanned those two outputs once per plugin. Both completed with
zero mesh errors. See `temp/mesh_metadata_stage_build.log`. Tests cover an
unchanged model, changed geometry, deletion, a broken rewrite, and all three
asset-stage entry points (including generated book art).

## Master asset metadata during import (2026-09-05)

The importer formerly loaded only its own bounds/collision caches and source
mesh folder. Translation has no mesh payload, so it imported without Nehrim's
bounds, collision, furniture seats/origin shifts, or mesh-authored door sounds.
Metadata now resolves transitive TES4 masters in load order, then the plugin.
Spawned LAND and navmesh workers receive the same ordered cache sources.

Collision merging uses each owner's bounds inventory to remove overridden
models before adding its collision entries: a collisionless child model must
not retain the master's collision. Loads also clear the previous plugin's
geometry and digests, including missing/empty loads. Source model resolution
selects the exact winning file before reading marker or sound data, so a
decorative/silent child correctly replaces a master chair/door. Master model
and SOUN records use the context's remapped source FormID keys.

The real cache probe loaded 14,173 bounds and 6,845 collision entries for
Translation from Nehrim, and 17,243 bounds and 10,063 collision entries for
UCWUS through Oblivion and Morroblivion. Tests exercise transitive inheritance,
collisionless overrides, independent worker initialization, furniture origin
shifts, and a silent door override. See `temp/master_mesh_metadata_probe.log`.
...
### Completing the interrupted Nehrim asset stage (2026-09-05)

The resumed character scope wrote all 1,559 planned hair variants; the source
`characters/ren/hair/ren_hair_005m_oc.nif` remains missing. The texture pass is
now callable as `asset_pipeline.copy_textures(asset_dir, plugin_dir)` so it can
resume without reconverting completed NIFs and hair. The ordinary mesh stage
uses the same function. A standalone resume preserves diffuse alpha unless
the mesh pass's opacity-use set is supplied.

The Nehrim resume copied 19,350 textures, expanded 473 luminance maps, fixed
73 landscape normals, and normalized 2,299 specular masks. Its book-art build
wrote 43 models with zero failures. Logs: `temp/nehrim_hair_resume.log` and
`temp/nehrim_texture_book_resume.log`.

The zero billboard-mode change needs no additional build in the current TES4
inputs: a header scan of 37,388 non-character/non-creature NIFs across the 21
targets selected 268 particle-billboard candidates (including mesh particles).
None combined mode zero, a direct geometry child, and a particle descendant,
which is the branch affected by the former `mode or 1` expression. The mode-12
Nehrim tower cap was rebuilt separately. Scope: `temp/zero_billboard_scope.log`.

### Inherited grass output lookup (2026-09-05)

The mod mesh pass reported 97 missing grass models for several plugins whose
base is Oblivion. `load_grass_model_paths` correctly includes master GRAS records,
but placement only searched the child output. The asset pipeline now passes
master output roots through the shared output-layout resolver. A model absent
from the child's source assets resolves to the master's profiled grass file.
If a local override exists but did not convert, or the master profile is absent,
it remains missing. This uses the same runtime path the inherited GRAS record
already names and creates no duplicate master asset in the child output.

### Bytes after terrain NIF footers (2026-09-05)

The Oblivion mesh pass found eight terrain LOD NIFs with bytes after the declared
blocks and root footer. Two inspected originals match their BSA bytes exactly:
`60.-64.-64.32.nif` ends its graph at byte 81377 with 234848 extra bytes;
`60.-32.-32.32.nif` ends at 315491 with 22698 extra bytes. Both have a valid
one-root footer and strip indices within their vertex arrays. The extra bytes
are outside the NIF object stream, not additional referenced geometry.

`nif_input.read_source_nif` detects that boundary, removes the invalid trailer
from its input buffer, and reparses the complete bounded stream with normal
link validation. Archives remain intact. Conversion reports the removed byte
count and writes the repaired graph. Round-trip tests retain every vertex and
triangle index; truncated footers still fail. No logger filters are involved.

### MOPP welding crash before scale retry (2026-09-05)

The full mesh build opened a Windows access-violation dialog in the MOPP bridge.
Reproducing `clutter/artsupplies/paintbrush01.nif` confirmed the original-size
input (extent 0.034 hu) exits `0xC0000005` during two-sided welding. The old
retry then succeeded at 0.34 hu, but only after the native process had already
crashed. Retrying after the fault did not fix the batch-build failure.

`run_mopp_bridge` now centers coordinates and applies a power-of-two scale
before its first native call, placing the longest extent in [64, 128). For
`q = (v - center) * factor`, the returned MOPP transform is restored with
`origin = center + native_origin / factor` and `scale = native_scale * factor`.
The CMS geometry and shape keys stay at their original size. Paintbrush01 now
uses one successful native call, with conditioned extent 69.632. Tests compare
the native and restored MOPP coordinates at every cube vertex for small,
translated, and large inputs; two castle meshes pass the independent key walk.

Unexpected native failures now raise with the saved input path and actual exit
status instead of falling through to collision removal. Windows reports such
failures to the parent without a blocking dialog; they remain conversion errors.
Empty bytecode, mismatched keys, or incomplete welding also fail conversion.

### Animation links and geometry flags (2026-09-05)

Current nifxml documents billboard mode values 8, 10, 11, and 12 in addition
to PyFFI's older enum. Nehrim's `oblivion/sigil/sigillighttowercap.nif` uses 12;
the old setter rejected it while copying a billboard, losing its orientation
mode. The enum now includes the documented values. Billboard demotion also no
longer treats valid mode 0 (ALWAYS_FACE_CAMERA) as a false/missing value and
replaces it with mode 1 (ROTATE_ABOUT_UP). Tests preserve 12 through a real
round trip and 0 on a generated geometry billboard under a particle parent.

Nehrim's `nehrim/refractioneffect.nif` has independently owned roots whose
objects are referenced by an earlier root's animation palette. Retargeting
each root immediately was too early for those cross-root pointers. Multi-root
files now receive a final retarget pass after all roots are converted. The
round-trip test preserves the source palette's complete target-name/null list,
and every non-null target belongs to the serialized graph.

Oblivion effect palettes also referenced editor-marker models that the converter
deleted, and billboard nodes replaced without retargeting their palette entries.
EditorMarker geometry now survives, explicitly hidden (the tested TES4 source
has flags 14, so the marker name supplies the editor-only visibility). Hidden
node flags survive traversal. Billboard replacements join the same identity map,
including billboard-to-node-to-fade chains. Round-trip checks cover
`sefxsparkledeme03.nif` and `seflamesofagnondem.nif`: marker geometry is present
and hidden, and every palette entry points to a written block.

The particle rebuild exposed dangling controller targets in Midas fire/ice/shock
streaks and `midasdestructionend.nif`. Root replacement had repaired only the
root's controller chain; nested controllers still targeted the discarded node.
Geometry replacement also missed `NiMultiTargetTransformController.extra_targets`.
`nif_links.retarget_links` rewrites owning and weak references by object identity,
including link arrays, while leaving unmapped external pointers intact. It visits
only fields containing links, not vertex data. Round-trip tests verify the root
target and the converted `RefractiveSphere` target survive serialization.

`midasstarbase.nif` exposed a separate PyFFI schema error: the high byte of
`NiGeometryDataFlags` was declared as a two-value enum. Material bits are valid
there, so copying geometry rejected and lost them. The backing field is now an
unsigned byte, preserving flags on copy/read/write. See the authoritative
[Niftools geometry flag definitions](https://raw.githubusercontent.com/niftools/nifxml/develop/nif.xml)
(`NiGeometryDataFlags` and `BSGeometryDataFlags`). Tangent generation now follows
particle-template ownership too and runs only with a UV/normal basis; invisible
UV-less markers keep their geometry without attempting an undefined tangent
calculation. Tangent failures are reported as conversion errors.

Batch conversion prints each data warning with its source-relative mesh path,
then category totals. A successful write alone does not establish clean conversion
or correct in-game animation. These changes have not yet been tested in game.

### Skin transform scale and animation build failures (2026-09-05)

Morrowind's Indoril greaves variants retain a reflected geometry-node frame.
Before solving their binds, the retargeter now separates a uniform signed scale
from any scaled/reflected rotation field on the geometry, skeleton root, and
bound bones, including their ancestor frames. Maskar's ancient bone bow stores
0.85 uniform scale in its `Bow:0` parent's rotation field; excluding ancestors
left that matrix malformed when computing geometry transforms. The complete
source matrix is unchanged. The bow and both greaves round trips
have proper rotation fields and satisfy `S @ B @ W = I` without warnings.
The bind-pose reader also uses the existing scale-aware matrix reader for S
and B; its previous hand-built matrices omitted each stored scale. A test with
S scale 2 and B scale 3 places the unit vertex at 6, then resets both binds.

Nehrim also authors affine matrices directly in static NiNode rotation fields:
`seelenbewahrer/klinge2.nif` has nonuniform scaling and reflection, while
`nehrim/natur/stamm01.nif` has scaled/sheared branches. `nif_affine` bakes those
static branch transforms into local geometry and child translations, retaining
world positions. It clones geometry data before changing an instance, transforms
normals by inverse transpose, and updates geometry bounds. Controllers, sequence
targets, skin bones, collision, unusual node classes, and shared node parents
protect a branch from flattening. Diagnostics for protected branches remain.
Tests compare every world-space vertex on both source meshes before/after the
repair; both converted meshes have no rotation warnings. A chandelier and guitar
probe also converts without warnings. Gameplay validation remains outstanding.

The female `armor/townguardcho/f/cuirass.nif` reproduced rotation warnings
(`error is 0.274641`) on the retargeted hand frame. `_np_to_nif_node` and
`_write_skin_transform` copied the full scaled 3x3 matrix into rotation and
forced the separate scale field to 1. They now factor the uniform scale out
using the signed cube root of the determinant; the full matrix is preserved.
The inverse reader includes the stored scale. The converted cuirass has valid
bone rotations and passes `S @ B @ W = I` for every checked skin binding.

Behavior-generation exceptions formerly left `converted=True`, so the batch
silently accepted an object with no animation graph. A controlled compiler
failure reproduced that success report. Such a mesh now reports `ANIMATION`
with the original exception and is not written. The real secret-wall graph
test still builds all four AMD64 HKX files and attaches its BGED correctly.

### NIF 4 geometry (2026-09-05)

The same reader also supports the shipped NIF 3.3.0.13 radius marker and
4.2.1.0 map marker. Both previously stopped at the version whitelist despite
reading to their exact file ends. The map marker's old NiKeyframeController
also lost its direct key data when the version changed. Legacy controllers
now become NiTransformController with NiTransformInterpolator/NiTransformData
before upgrading the stream. Round-trip tests preserve all key values, timing,
targets, geometry, and hidden shape flags; invisible markers stay invisible.

PyFFI declares `NiGeometryData.num_uv_sets` twice with different types, then
initializes both through the first (byte) backing field. NIF 4 requires a
ushort there. `marker_arrow.nif` reproduced the one-byte shift: its authored
22 triangles became 5,632, ending in an EOF read failure. The version-aware
field now matches [Niftools' NiGeometryData layout](https://raw.githubusercontent.com/niftools/nifxml/develop/nif.xml).
Legacy triangle and strip presence is also implicit through 10.0.1.2; set
the newer presence flags before converting, or the valid geometry is dropped.
The NIF 4.0.0.2 source now converts and reads back with its 39 vertices and
22 original triangle indices. Marker models are intentionally invisible in
game: the output shape stays hidden and has no render shader. Geometry
preservation is a parser/conversion check, not a request to display markers.
Targeted tests also check modern field widths
and mesh-particle serialization. Rendering is not yet verified in game.

### Mesh-particle templates (2026-09-05)

`NiPSysMeshUpdateModifier.meshes` owns scene templates outside the normal
`NiNode.children` traversal. Midas's `magiceffects/midastyraeleffect.nif`
reproduced a write failure in an untouched template morpher: `array size (0)
different from to field describing number of elements (6)`. Convert those
templates through the geometry path and bake their direct morph controllers
using the existing visibility-swap approximation, preserving the source clock
and loop flags. Keep `NiMeshPSysData` and its derived payload; replacing it with
plain `NiPSysData` discards the mesh-particle configuration. Its serializer must
write the corrected Skyrim base prefix followed by the derived fields.

The failing Midas mesh now round-trips and its targeted pipeline build writes
to output with zero conversion errors. Tests check the derived binary payload,
template geometry, and animation clocks, plus the existing Oblivion fire case.
Gameplay remains unverified. Read/write errors now retain their exception text
instead of reporting only `RD`/`WR`.

This does not repair continuous morph animation. Maskar's `torchwilli*` mesh
emitters still fail on a direct `NiGeomMorpherController` whose interpolation
weight array was not resized for Skyrim. Filling that array alone would only
repair serialization: Skyrim has no equivalent morpher controller. These
emitters sample an animated surface, so visibility-swapping rest and target
meshes changes the authored emission positions between keys.

Read-only CK disassembly traced `NiPSysMeshEmitter` slot 45 to RVA `0x27B8E10`
and its face-surface helper to `0x27B8ED0`. The helper reads packed triangle
vertices, not skinned vertex output. This rules out treating GPU skinning as
a demonstrated repair. The running game's corresponding path remains to be
verified; CK has two additional base virtual slots. Probes are retained in
`temp/ck_emitter_generate.log` and `temp/ck_emitter_surface_full.log`.
...
- **PyFFI warning capture (2026-09-01):** toaster/spell traversal text is
  logged by PyFFI at WARNING level even though it is progress (`--- pass ---`,
  `~~~ block ~~~`, `adding tangent space`, optimization/skin progress). Workers
  now capture that logger, discard only the enumerated progress prefixes, and
  return categorized counts for real data warnings (NaNs, invalid block links,
  collision, skin, particle, controller, palette, stream, and enum problems).
  Unknown messages remain visible under a leading-type category; the filter
  never drops an unrecognized warning.
...
### Generated centered door pivots (Morroblivion fix, 2026-09-01)

Morrowind-to-Oblivion door assets may synthesize `Open`/`Close` as a pure
two-key Euler-Z rotation while leaving the controlled node at the **center** of
the leaf. Skyrim correctly plays that controller, so the panel spins through
the doorway instead of swinging from a hinge. The Morrowind_ob.esm DOOR model
census found 41 vertical leaves with this centered pivot, alongside 41 already
edge-pivoted leaves and 17 horizontal/bulky models that must not be treated as
vertical doors.

`_fix_centered_door_pivot` recognizes the authored generator fingerprint, not a
plugin or path: paired reverse `Open`/`Close` clips, one two-key Euler transform,
X/Y rotation fixed at zero, a 0.75–2.2 rad Z swing, and no translation keys. It
then requires a thin vertical leaf whose origin lies at least 30% of its width
from either edge. A small detached part at one edge is the handle indicator, so
the opposite edge is the hinge; without one, the authored origin's nearer edge
is used (low edge only breaks an exact symmetry). For every rotation key it adds
the exact compensation
`T(t) = T0 + Rclosed*P - R(t)*P`, keeping hinge point `P` stationary while
preserving both closed and open rotations. Existing edge pivots, trapdoors,
multi-track gates and ordinary animations remain byte-semantically unchanged.
The focused conversion check measures 0.000004-unit hinge drift on
`ExUcommonUdoorU01`; in-game confirmation is pending.
...
- **Degenerate collision hulls crash the MOPP bridge**: the bridge access-violates (`rc 3221225477`) inside "computing two-sided welding" on very small hulls — Havok divides by near-zero edge lengths. Verified by scale sweep on `inucaveuplant00`: ×1 crashes, ×10/×100/×1000 all succeed. The duplicated reversed triangle in that mesh is *not* the trigger (A/B tested — crashes with and without it). **This is not Morroblivion-specific**: vanilla Oblivion `paintbrush01/02/03` (0.034 hu) hit it too.
  - **Fix is a scaled rebuild, not a drop** (`cms_builder.build_cms_collision`): MOPP encodes geometry as `(v - origin) * scale`, so building the bytecode over vertices scaled by `k` and storing `origin/k` with `scale*k` is an **exact** restatement for the original geometry — no approximation, and the CMS chunk data stays at native scale. The historical k=10/100/1000 retry is superseded by conditioning before the first call (2026-09-05, above). Verified on paintbrush01: MOPP origin reproduces the source AABB minimum on all 3 axes, decoded CMS AABB matches the source, `walk_mopp` returns 0 errors and its terminal key set equals the CMS key set.

...
## Remaining native-command investigations (2026-09-05)

The two Morroblivion Norvayne scripts still fail compilation (`IsYielding`).
Oblivion's yielding getter tests combat-controller states 5/6; Skyrim fleeing
or bleedout flags are not demonstrated equivalents.

## INFO editor drafts without executable bytecode (2026-09-05)

Oblivion's lazy INFO result-script loader at `0x531290` reads SCHR, SCDA and
SCRO from the last owning plugin; it never reads SCTX. It resets the shared
Script before loading, so an override without SCDA does not inherit the
master's bytecode or execute its own stored editor text. The export preserves
both text and exact bytecode/header bytes. The importer retains an explicitly
bytecode-less draft as `ResultScript.SourceText`, outside executable conversion.

Fresh INFO exports across the 21 installed TES4 targets identified two such
drafts containing commands: Translation `000011D3` and Morroblivion `013233F0`.
Translation's draft names nonexistent `SchattenrufKreaturMusik01`; its Nehrim
master has a different, compiled result script. Applying the source engine's
loading rule fixes Translation compilation: 1,325/1,325 scripts, zero failures.
The Morroblivion draft contains `setstage fbmwBMWomanScorned 30`. Preserving
either draft as executable Papyrus would introduce behavior absent in TES4.
Engine probes are in `temp/info_result_script_loader.log` and
`temp/translation_compiled_probe.log`; lifecycle regression tests cover a draft
and genuine compiled bytecode. This is not an in-game verification.

## INI command investigation (2026-09-05)

Retained command names also require implementation checks: Creation Kit's
HasFlames, AddFlames, RemoveFlames, SetAllVisible, SetSceneIsComplex, Wait,
StopWaiting and IsWaiting execute handlers only return success. Forwarding
converted scripts to those handlers would not restore behavior. Oblivion's
AddFlames (`0x4E26F0`), RemoveFlames (`0x4DE1C0`) and HasFlames (`0x4DE320`)
operate on the model's FlameNode children, with a separate FlameCap visibility
branch. Explicit AddFlames decodes the first socket character; that must not
be confused with initial model loading's socket-name table. Probes are in
`temp/retained_warning_handlers.log` and `temp/flame_engine_probe.log`.

INI commands remain unresolved. xOBSE's numeric INI setter changes in-memory
typed settings, not files. Skyrim's INI and preference collections provide the
corresponding typed storage, but several authored names have moved or vanished.
Executable strings confirm `bAllowScriptedAutosave:SaveGame` rather than
Oblivion's `:General`, and `fMouseHeadingSensitivity:Controls` rather than
`fMouseSensitivity:Controls`. Names alone do not establish identical behavior.
The old global-time-multiplier INI entry is absent; the engine exposes the
`SetGlobalTimeMultiplier` command instead. The autosave preference mechanism,
mouse-value semantics, and timer setter still require runtime verification.
No warning was removed on the strength of a guessed setting substitution.

## Scripted rotation (2026-09-05)

Oblivion's `Rotate` command at `0x510620` reads the requested reference Euler
axis, converts its signed integer degrees-per-second operand to radians, and
multiplies by the frame delta at `0xB33E9C`. It wraps the angle to one revolution,
writes the reference angle, and updates the loaded scene node. An unloaded
reference is unchanged. See `temp/rotate_engine_probe.log` for the measured
instruction sequence; constants are degrees-to-radians at `0xA31C78` and a full
turn at `0xA3D5B0`.

The converter previously discarded Rotate. It now uses a loaded-reference
polyfill that preserves the other axes, normalizes the angle, and invokes the
engine's SetAngle native. GameMode/ScriptEffectUpdate use the measured elapsed
time between their converted updates, including scripts that never explicitly
call GetSecondsPassed. Other events use Skyrim's frame delta through CommonLib's
GetSecondsSinceLastFrame (Address Library IDs 523660/410199). Converted updates
run at 0.1-second registration intervals, so this restores angular progress but
does not reproduce Oblivion's per-frame visual smoothness. In-game verification
remains necessary.

## Forced running and sneaking (2026-09-05)

Oblivion `SetForceRun` (`0x50C290`) and `SetForceSneak` (`0x50C390`) write
the actor's force flags at `0xC8` / `0xC9`; they do not modify an actor value
or request an immediate animation. Skyrim retains separate `forceRun` and
`forceSneak` bits in `ActorState2`. The runtime now changes those bits and
the existing getters read them. The previous SetForceRun→SpeedMult mapping
was wrong: it altered movement speed instead of selecting the movement mode.
Targeted source tests and a native probe verify both toggles, independent
flags, and unchanged speed. This does not repair SpeedMult changes already
written by an older converted script into a saved actor.

## Current furniture identity (2026-09-05)

Oblivion `IsCurrentFurnitureRef` (`0x4F6640`) compares the actor process's
furniture reference; `IsCurrentFurnitureObj` (`0x4F66A0`) compares its base.
The runtime reads Skyrim's actual occupied-furniture handle through
`Actor::GetOccupiedFurniture` / `AIProcess::GetOccupiedFurniture`, which reads
`MiddleHighProcessData::occupiedFurniture`, and preserves those two identities.
The legacy Skyrim condition names alone were insufficient proof: a census of
vanilla PACK, INFO, IDLE and QUST found no uses of either condition ID.

## Copying and appending names (2026-09-05)

xOBSE `Commands_Inventory.cpp::Cmd_CopyName_Execute` and
`Cmd_AppendToName_Execute` resolve references to their base forms. A missing
optional target falls back to the calling reference's base. They modify the
target's direct `TESFullName`, without a persistent change flag; they do not
edit map-marker extra data or an exterior cell's parent world name.
`CopyName` uses OBSE's `<no name>` fallback for an unnamed source, whereas
`AppendToName` appends to the actual field, including an empty field. Separate
runtime methods retain these semantics instead of composing display-name APIs.

## Process-start and load notifications (2026-09-05)

xOBSE `Commands_Game.cpp` implements `GetGameRestarted` with a process-lifetime
set of notified scripts. `GetGameLoaded` has a separate set cleared on load
and new game (`Hooks_SaveLoad.cpp::DoLoadGameHook` and
`Hooks_Gameplay.cpp::HandleNewGameHook`). Neither is an ordinary saved Boolean.
The runtime identifies the calling script definition from its non-native VM
stack frame, so instances share a notification. Separate unsaved sets preserve
the two lifetimes; only the load set clears on load/new-game messages. A native
probe checks repeated calls, separate scripts, load and new-game transitions;
conversion tests verify that guarded initialization bodies remain executable.
The VM integration still needs an in-game check.

## Current or last-ridden horse (2026-09-05)

Vanilla `GetPlayerHasLastRiddenHorse` is a global Boolean query, distinct from
OBSE's `GetPlayersLastRiddenHorse` reference query. Oblivion's command entry
`0xB10150` calls `0x505F00`, then `0x4F8630`, which tests the player's current
mount (`GetMountedHorse`, vtable offset `0x380`) OR `lastRiddenHorse` (`0x1E0`).
The runtime uses Skyrim's `GetMount` and `QLastRiddenMount`; an earlier comment
claiming Skyrim has no last-ridden state was incorrect. Script conditions and
numeric assignments retain their Boolean-to-integer conversions.

## Effect-code strings (2026-09-05)

`MagicEffectCodeFromChars` / `MECodeFromChars` and `MagicEffectFromChars`
accept exactly four bytes, interpreted little-endian, and require that the
effect exists. xOBSE `Commands_MagicEffect.cpp::MagicEffectFromChars_Execute`
does that lookup; treating an arbitrary four-character string as a valid
effect would be wrong. The runtime resolves against converted source-effect
metadata. `GetMagicEffectChars` / `GetMagicEffectCharsC` return the four bytes
for an existing effect and the string `"0"` for a missing effect, matching
xOBSE's `GetMagicEffectChars_Execute`. Signed high-bit codes are preserved.
Targeted conversion tests and a native probe cover aliases, dynamic strings,
invalid lengths, unknown codes, embedded nulls and a high-bit round trip.

## TrapUpdate is obsolete in Oblivion itself (2026-09-05)

The vanilla command table entry at `0xB0F2A0` registers `TrapUpdate` with the
handler `0x977C50`. Its complete executable body is `mov al, 1; ret`: successful
command execution, with no calls, argument reads or writes to game state.
The generated source retains an explanatory comment instead of claiming an
unsupported trap operation. Trap movement/physics must come from the other
authored commands and NIF controllers; this finding does not validate them.
Reproduce with `tools/disasm/oblivion_exe_probe.py --exe <Oblivion.exe>
--disasm 0x977c50 --count 8` and the table's handler pointer at `0xB0F2B8`.
The PC `AddAchievement` command's entry at `0xB0FC50` points to the same
empty handler. This does **not** apply to Nehrim's `UnlockAchievement`:
[its separate OBSE plugin](https://github.com/FeanorTheElf/obse_nehrim_steam)
implements real Steam achievement delivery, which remains unsupported.

## Saved base-record commands (2026-09-05)

Oblivion `SetItemValue` (`0x506810`) resolves the reference's base object,
casts it to `TESValueForm`, and calls `0x4703C0`: that setter writes the integer
value and marks base change flag 8. The conversion uses the runtime's saved
gold-value store with an integer argument, preserving values above Float's
exact integer range. `SetActorFullName` (`0x512BB0`) writes the actor base's
full name and marks its name change; the Skyrim native similarly updates
`TESNPC::TESFullName` and marks `TESNPC::ChangeFlags::kFullName`. These commands
do not create per-reference display-name overrides. Conversion tests and the
native build pass; persistence still needs an in-game save/load check.

## Calls through function variables (2026-09-05)

An OBSE `Call` target may be a ref variable or an array element, including a
script loaded from an optional plugin. Static calls retain their concrete
`TES4Call` signatures. Dynamic calls cast the target Form to `TES4Function`
and call a uniform `TES4Invoke` wrapper generated on every function host.
Arguments/results use typed collection values; indexed arguments copy their
stored type. Both execution and return delivery remain normal Papyrus calls,
so there is no native callback waiting outside a saved VM stack. Nested
functions preserve `TES4_Caller`. UCWUS's previously rejected variable-held
call compiles with this path; optional Extended UI integration still requires
that plugin's converted function host to be present at runtime.

## OBSE effect counts and array append (2026-09-05)

`GetMagicItemEffectCount` and its documented alias `GetMIEffectCount` use the
authored effect-list metadata, like the existing indexed effect queries.
xOBSE `Commands_Magic.cpp` defines both names; omitting the `Item` spelling
left real calls unsupported despite an implemented count handler.

`ar_Append` inserts at the current size of a packed array and returns 1 on
success, 0 for an invalid target or map. Native typed append methods retain
integer, float, string, form and nested-array values in the existing saved
Papyrus collection pages. Appending an indexed entry copies its runtime type
directly, including when source and destination are the same array.
The contract comes from xOBSE `Commands_Array.cpp`, `Cmd_ar_Append_Execute`.
Conversion tests and the native build pass; save/load behavior needs an
in-game check.

## OBSE key-state queries (2026-09-05)

`IsKeyPressed` uses Windows virtual-key codes and `GetAsyncKeyState`.
`IsKeyPressed2` reads the game state after simulated/disabled input, through
SKSE `Input.IsKeyPressed`. `IsKeyPressed3` reads physical DirectInput keys
before SKSE filtering. These are different contracts in xOBSE
`Commands_Input.cpp` and `Hooks_Input.cpp`; they must not share one mapping.
The runtime observes the original device's existing `GetDeviceState` call
through the COM interface returned by SKSE's forwarding `QueryInterface`.
It does not perform extra polls that would consume mouse-wheel movement.
As in xOBSE, wheel codes 264/265 use the current game state. Native build and
conversion tests pass; disabled-key behavior still needs an in-game check.

## Mesh facts for dependent scripts (2026-09-05)

Script workers load mesh bounds and physics flags from the complete master
chain, followed by the plugin's own cache. A plugin without meshes does not
need a private bounds cache: UCWUS uses Oblivion and Morroblivion's caches,
and Translation uses Nehrim's. Missing metadata for owned meshes still warns.
Each load clears prior plugin state, including empty or failed loads; otherwise
the parent and spawned workers could disagree about whether a trap is held.
Master directories resolve through the imported-mod registry, including nested
record folders. Regression tests cover master trap releases, overriding flags,
and state clearing. Missing/stale metadata for owned meshes remains unresolved
until those meshes are scanned.

## Additional DLC conversion defects (2026-09-05)

`PosWorld` is the engine alias for `PositionWorld` (Oblivion command table
at `0xB0DB80`, handler `0x508C30`). Both now retain the authored worldspace;
the old handler discarded it. The native move resolves that world's cell and
passes the rotation in radians, matching the handler's direct angle copy.
The runtime and targeted Papyrus compile succeed; cross-world movement still
needs an in-game check.

SCRO rename recovery now distinguishes quoted form operands from display text
using command argument types. Spell Tomes' `DLCTomeSpellLeechVitality` binds
to compiled reference `01000CE9` (`DLCTomeSpellLeechVitality3Journeyman`);
message labels `Yes`/`No` previously prevented the unique rename from resolving.
INFO exports retain SCRO tables too. All available TES4 inputs were re-exported.

`GetAmountSoldStolen` emits a Float global read, so integer assignments must
cast the full expression. Authored abbreviated ranges such as `value >= 0 &&
<10` recover the repeated comparison subject rather than discarding the bound.
This is source-recovery intent, not a claim that the malformed text was valid
Oblivion syntax. The four affected DLC scripts pass a targeted Papyrus compile.
...
### Remaining mod compiler failures (2026-09-05)

Recover a missing numeric local declaration only when the source both writes
and reads that name, every assigned value is numeric, and no authored record,
global, declaration, or SCRO alias owns it. This handles UVIII's `SkipAnim`
flags without turning real globals or commands into fabricated variables.

Numeric slots used for both flags and reference identity now store the current
FormID through `TES4SKSE.NumericFormID`; mixed equality comparisons read the
same identity. Previously the reference write was commented out and the
comparison did not compile. Null remains zero and reference/null comparisons
remain `None` tests. These Int values are runtime IDs, not saved Form properties;
they do not gain FormID rebinding when the load order changes. The UVIII victim
lock, two undeclared-local scripts, and race-spell query compiled in a targeted
five-file check including the helper. Gameplay is not verified.

`Race.GetSpellCount/GetNthSpell` are SKSE64 natives and need compiler overlay
declarations, not replacement implementations.

`GetGameDifficulty` reads CommonLib's `PlayerCharacter.GameStateData.difficulty`.
The conversion policy maps Skyrim's indices 0,1,2,3,4,5 to -1,-0.5,0,1/3,2/3,1:
both endpoints and neutral Adept survive, but the two games' damage multipliers
are not identical. xOBSE `Commands_Game.cpp` defines its source range as -1..1.
Skyrim's `fDifficultyDamageMultiplier` exists as a compiled-in setting even
though no override appeared in the measured Skyrim.esm GMST dump: Creation
Kit's initializer references it at RVA 0x010d75af. Missing GMST records alone
do not prove an engine setting is absent.

### Numeric actor values and velocity (2026-09-05)

`GetVelocity` and the vertical/full setters use the actor's character controller,
with `bhkWorld` scale conversion and world locking. xOBSE's
`Commands_Physics.cpp` is the source contract: setting vertical velocity preserves
the horizontal components. Parser arities keep negative setter arguments and
arithmetic following a getter inside the correct expression.

`SetAVC`/`ModAVC` resolve constant TES4 codes through the named actor-value
converter. Runtime codes use a generated `TES4ActorValues` helper shared by the
master and its plugins. `GetAVForBaseActor[C]` reads the specified NPC base (or
the receiver's base), as xOBSE's `Commands_ActorValue.cpp` does; it must not feed
a fabricated zero into a later health setter. Attribute codes retain the
existing attribute policy documented below. Main and Maskar script builds
compiled 17,238/17,238 and 1,956/1,956 scripts respectively in this session.
This does not resolve the remaining TODO/NE warnings or verify gameplay.
...
## Runtime GMST access targets SKSE64 (updated 2026-08-29)

`Game.GetGameSettingFloat/Int/String` are vanilla natives. `Game.SetGameSetting*`
is SKSE64, and the converter now emits the typed SKSE64 setter for OBSE's
`SetNumericGameSetting` / `SetStringGameSettingEX`. `convert.py` builds a
temporary compile-header overlay from the installed vanilla headers plus the
needed official SKSE64 declarations. The overlay is compile-only; it is never
packaged and never replaces Skyrim's base scripts.

Two rules follow:

- **The READ must use the same channel as the WRITE.** These scripts use the
  save/restore idiom ("remember the old value, set a new one, put it back"), so
  both sides now operate on the global GMST through `Game.Get/SetGameSetting*`.
...
Settings absent from Skyrim keep a `;TODO` marker — a call that compiles and
silently does nothing is the dangerous outcome, not the honest one.

Authored comments containing the word `TODO` are different: the converter
preserves their text but relabels the token as `Source note:`.  Therefore every
remaining `;TODO:` in generated Papyrus is a converter-owned unresolved
translation, not a mod author's reminder.
...
- A standalone `GetSecondsPassed` statement is the TES4 idiom for resetting the
  elapsed-time baseline.  In a real-time GameMode poll it becomes
  `TES4_LastTick = Utility.GetCurrentRealTime()` instead of a malformed or
  silently discarded expression.
- `OnKnockout` becomes Skyrim's parameterless `OnEnterBleedout`.  `OnMurder`
  remains a distinct `OnMurder(Actor akKiller)` event: Skyrim retains the event
  in its engine table and the Papyrus compiler accepts that signature, so it
  must not be folded into `OnDeath` and make authorized kills indistinguishable
  from murders.
...
| `GetStringGameSetting` | `Game.GetGameSettingString` | Vanilla Skyrim native. |
| `Get/SetNumericINISetting` | `Utility.Get/SetINI{Bool,Int,Float}` | Getter is SKSE64; writers are vanilla. Literal b/i/f setting prefixes select the type. |
| `GetCrosshairRef` | `Game.GetCurrentCrosshairRef` | SKSE64. |
| `GetObjectType` | `TES4SKSE.GetObjectType` | Dereferences the placed ref, reads SKSE64 `Form.GetType`, translates to OBSE's TES4 code. |
| `IsWeapon` / other form tests | `TES4SKSE.GetBaseType` + `Form.GetType` | Tests the base form rather than the REFR/ACHR wrapper. |

Genuinely absent (inert `;NE:`): OBSE UI/menu paths (`get/setMenu*`; Oblivion XML
paths cannot address Skyrim Scaleform), Windows-virtual-key `IsKeyPressed`, stock
SKSE64's unregistered `DisableKey`/`EnableKey`, console commands without native
counterparts, `SkipAnim`, `getPackageTarget`, and `UnlockAchievement`.
`IsKeyPressed2/3`, mapped controls, typed INI access, crosshair lookup, GMST
strings, form type/name/playability, and the supported `sv_*` string operations
now emit vanilla/SKSE64 calls. An unsupported heterogeneous OBSE `forEach … loop`
still suppresses its whole body because a valid typed iterator cannot be formed.

A cross-script access to a variable the owner never declares is **dangling in
the original mod**.  Oblivion ignored it; Papyrus fails the whole file, so a
write is commented out and a read becomes `0`, both with `;NE:` diagnostics.
Owner lookup must use `resolve_property_formid`, because TES4 source can omit a
leading digit that Papyrus-safe EditorIDs retain.  Akatosh Mount's
`SSMountedFlightQuest*.Summoned` accesses are this case: the records are named
`1SSMountedFlightQuest*`, and none of their attached scripts declares
`Summoned`.  Do not synthesize the missing field.
...
- **Punctuation-only source lines are banners, not converter failures.** TES4
  uses bare `-----` and `:====` separator rules; preserve them as ordinary
  comments instead of emitting live prefix expressions or `;TODO:` markers.
- A known zero-argument command may also be wrapped in quotes in released TES4
  source (`"EnableLinkedPathPoints"`). Dequote only when the token is a known
  command, then use its normal mapping; do not treat arbitrary strings as code.
- **A leading `&&`/`||` continues the preceding physical source line.** Join it
  before expression conversion, because Papyrus has no implicit continuation.
- Bool-returning calls used on either side of arithmetic need an operand-local
  `as Int` cast. Casting the whole sum is too late, and inferring a nested
  `GetBaseForm(...).IsPlayable()` as Form produces the invalid `Int != None`.
...
- **A signed command argument is not subtraction.** In
  `(marker.GetPos x) + Rand -32 32`, the arithmetic scanner must skip the sign
  on `Rand`'s first operand, split the surrounding `+`, and recursively emit
  `Utility.RandomFloat(-32, 32)`. The signed-first-argument allowlist is
  deliberately arity-aware: `GetCurrentTime - 1` remains subtraction.
- **A command name can still be an authored numeric local.** Released CSE
  source sometimes omits a declaration retained by its compiled script.
  A command-named target of `set X to <numeric literal>` is recovered as a
  local only when every assignment is numeric and no form named X exists.
  This preserves UVIII's `SkipAnim` flag instead of turning its assignments
  and conditions into malformed comments.
- **`ObjectHealth` and `CurrentHealth` are different xOBSE state.**
  `Get/SetObjectHealth` reads or mutates `TESHealthForm::health` on the base
  weapon/armor; Skyrim removed base item durability, so these emit a compiling
  `;NE:`/zero rather than falsely changing tempering. `Get/SetCurrentHealth`
  remains the per-instance 0..100 condition and maps to SKSE64's
  `ObjectReference.Get/SetItemHealthPercent`.
...
- **OBSE string variables are Papyrus `String`.** `sv_Construct` assigns the
  formatted value; `sv_Destruct` clears it; length/find use `StringUtil`; and
  compare/count/erase/insert/replace use the stateless `TES4SKSE` helper.
- No direct Papyrus equivalent, emitted inert with `;NE:` — heterogeneous OBSE
  arrays/maps (`ar_*`, `forEach` when its element type cannot be proven),
  path-based music (`StreamMusic` and Nehrim's bundled `emc*`
  plugin; Skyrim music is MusicType-based),
...
* Every writer routes through the polyfill.  The `setdestroyed` command row in
  `constants.py` has an `OBJREF` subject so implicit TES4 `Self` becomes
  `GetTargetActor()` in an `ActiveMagicEffect`; passing the effect object does
  not compile.  A direct-native mapping would bypass the mirror.

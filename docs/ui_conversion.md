...
Anything past a reskin needs an authored Skyrim Scaleform movie and target
paths. The converter now uses SKSE64 for script capabilities, but that does not
make an Oblivion XML `get/setMenu*` path meaningful to Skyrim's `UI.*` API; see
[skse_conversion_audit.md](skse_conversion_audit.md).

...
## Linked path-point commands: verified mechanisms (2026-09-05)

`DisableLinkedPathPoints` / `EnableLinkedPathPoints` call Oblivion `0x4E51F0`,
which visits the loaded interior pathgrid or loaded exterior cell grids.
`0x4E5170` looks up the calling reference in `TESPathGrid::linkedPathPoints`
and toggles those points; disabling marks the owning CELL with change flag
`0x01000000`. The exporter preserves this authored relationship as
`RefMap[i].Reference` and `RefMap[i].Point[j]` from PGRL. This is not a model
visibility toggle or a nearest-reference association.

Skyrim.esm defines `L_NAVCUT` (`000E7518`) with collision index 49 and the
Navmesh Obstacle flag 4. Vanilla references `000F7E9A`–`000F7E9C` use base
`00000021`, XTRI=49, XPRM box primitives (type 1), and enable parent
`000C867B`. xEdit's XPRM definition and these binary records agree on its
32-byte layout: bounds, RGB, unknown float, primitive type. Nav-cut primitives
therefore provide an engine mechanism for invisible navigation obstacles.

The linked-point conversion remains unimplemented. A point-centered box must
not be treated as a full fix without proving it prevents bypass on the generated
corridor mesh and preserves unrelated routes. The source relationship and the
runtime mechanism are verified; the required geometry mapping is still open.
...
Cached and cold geometry is oriented **after float32 quantization**. A triangle
that is barely positive in float64 can become clockwise when packed; correcting
only before packing makes the cold build pass and the cached build fail with a
different winding. Adjacency is also geometry-aware: two triangles sharing an
edge are not linked when their normals are nearly opposite (`dot < -0.9`), so
an accidental folded seam cannot become an engine-walkable portal.
...
2. **Every NVMI entry declared zero connectivity.** Contract verified against
   ...
   A triangle is not exclusive to one door: Skyrim.esm contains multiple doors
   bound to the same NAVM triangle. `_build_door_links` therefore may reuse the
   best triangle for another door; a `used_tris` gate incorrectly dropped
   otherwise valid links in tight multi-door thresholds.

   Collision-backed sheet unions can return the same indexed triangle more
   than once. Before adjacency and door selection, exact indexed faces are
   deduplicated and ledge endpoints are remapped to the kept face. Leaving both
   copies makes all three edges of each triangle target the other copy, which
   the CK reports as `DUP_EDGE_TARGET` (measured on Midas' arena cell).

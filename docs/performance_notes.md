...
## Windows virtual-environment worker startup

On Python 3.14.6 in a Windows virtual environment, the old
`configure_multiprocessing()` switched the executable to the venv's
`pythonw.exe` redirector. This bypassed CPython's `WINENV` branch in
`multiprocessing/popen_spawn_win32.py` (bpo-35797), which normally launches the
base interpreter and supplies `__PYVENV_LAUNCHER__`. The process used for handle
duplication was then the redirector instead of the actual worker. A two-file
paintbrush conversion stalled: surviving workers had completed zero tasks,
while a bounded two-worker reproduction repeatedly exited on queue EOF/OSError.
The same paintbrush converted successfully in a single process.

The fix leaves executable selection to CPython and wraps only this spawn
module's WinAPI interface to add `CREATE_NO_WINDOW` to `CreateProcess`. Other
WinAPI callers retain the original module. A real two-worker test verifies
result delivery and matching venv prefixes; the two-paintbrush pool probe also
completes after the change. Capture logs are under `temp/paintbrush_pool314*`.

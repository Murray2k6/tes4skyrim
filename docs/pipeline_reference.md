...
`--scripts-only` converts vanilla Oblivion and OBSE source to Papyrus. Calls
with real SKSE64 counterparts target the SKSE64 API. The compiler creates a
temporary augmented-header overlay under the plugin's script output and removes
it after compilation; the overlay is never packaged. A converted plugin that
uses those calls therefore requires SKSE64 at runtime.
...
`--mesh-subdirs` limits a mesh rebuild to relative folder prefixes and accepts
either slash style. A root name still works, while a nested prefix avoids an
expensive whole-tree pass:

```bash
python convert.py -f Morrowind_ob.esm --meshes-only --mesh-subdirs morro/d
```

The filter also applies to hair variants, grass copies, and book inventory
models using their authored source model paths. Scoped builds copy and process
only the selected meshes' texture dependencies (including implicit shader
siblings). They merge texture manifests with prior builds. Book filename
collision resolution uses the complete model set before filtering, so names
remain identical to a full build. Mesh, hair, or book-art errors fail the stage.
...
`--pack-zip-only` works before BSA packing and for asset-only mods. It ships
loose assets unless a successful BSA pack's `.bsa-pack.json` receipt matches
the current source file list, sizes, modification times, and archive states.
Adding, removing, or rebuilding an asset invalidates that receipt. This is local
build bookkeeping, not a shared cache key, and the receipt is not shipped.
Older BSAs without receipts fall back to the available loose assets; an
archive-only output with a loader plugin can still be packaged.

SKSE files always ship loose. Root INI files and text notices are included.
ZIP output is written to `.zip.part` and replaced only after a successful close,
so interrupted packaging preserves the previous completed ZIP. Grouped plugins
still produce one ZIP named after their shared mod folder.

Verified with real BSArch payload round-tripping, asset-only input, source
add/change/delete, changed archives, grouped output, and interrupted writes.
The `Morrowind_ob - UCWUS.esp` pack and ZIP stages completed with zero errors.
Existing conversion warnings in that mod are independent of packaging.

## Texture manifest validation

An existing, empty `textures_used.txt` is a completed mesh scan with no texture
references, not a missing scan. Texture selection still gathers references from
records and generated assets, including creature meshes. The packer now tests
manifest existence separately from its contents. Akatosh's empty manifest was
verified through packaging: all 30 referenced output textures are in its BSA.

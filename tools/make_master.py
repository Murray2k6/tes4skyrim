...
Usage:
  python tools/make_master.py Tamriel.esp ElsweyrAnequina.esp
  python tools/make_master.py --all-dependents Tamriel.esp
  python tools/make_master.py --clear Tamriel.esp        # revert to ESP
  python tools/make_master.py --check Tamriel.esp        # report only
  python tools/make_master.py --output-dir output Tamriel.esp
"""
import argparse
import os
import struct
    ...
    if enable and not args.check:
        ...
        if unflagged or missing:
            ...
            if need:
                ...
                print(f'  python tools/make_master.py {" ".join(ordered)}')
            if not args.force:

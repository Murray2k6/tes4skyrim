...
the thing being tested, and reset between runs.
    ...
    python tools/quest_labtest.py cast    --quest Charactergen
    python tools/quest_labtest.py setup   --quest Charactergen
    python tools/quest_labtest.py reset   --quest Charactergen --stage 42
    python tools/quest_labtest.py run     --quest Charactergen --seconds 60 \\
...
def cmd_cast(args) -> int:
    ...
    print('\n  test these with:\n'
          f"    python tools/quest_labtest.py setup --quest {q.get('EditorID')}")
    return 0
        ...
        with Bridge().connect(retries=2) as b:
            ...
            if caps.get('console'):
                ...
                add('console round trip', bool(out.strip()),
                    ...
                    'tools/game_input.py bootstrap once per session')
...
def _dialogue_reader(emit):
    try:
        from dialog_live import DialogueReader
        r = DialogueReader(0)

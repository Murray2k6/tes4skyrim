...
Usage:
    ...
    python tools/validate/cell_grid_check.py output/Oblivion.esm/Oblivion.esm

    # confirm the invariant on vanilla (expects 0)
    python tools/validate/cell_grid_check.py "<skyrim>/Data/Skyrim.esm"

    # also list duplicate grid squares within a worldspace
    python tools/validate/cell_grid_check.py <esm> --duplicates

    # per-worldspace grid extents + cells outside CK's legal coordinate range
    python tools/validate/cell_grid_check.py <esm> --extents

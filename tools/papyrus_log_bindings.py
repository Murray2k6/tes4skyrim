...
Usage:
  ...
  python tools/papyrus_log_bindings.py <Papyrus.0.log> --plugin Morrowind_ob.esm

  # Only one class, listing every property in it:
  python tools/papyrus_log_bindings.py <log> --plugin Morrowind_ob.esm \
      --declared Armor --verbose

  # Only failures whose target FormID does not exist at all (nullptr form):
  python tools/papyrus_log_bindings.py <log> --plugin Morrowind_ob.esm --missing
"""
import argparse
import os
import re

...
Usage:
    ...
    python tools/papyrus_tail.py list

    # mark the current end of the log; prints a cursor you pass back later
    python tools/papyrus_tail.py mark

    # everything appended since a cursor (the normal agent loop)
    python tools/papyrus_tail.py since --cursor 502545

    # last N lines right now
    python tools/papyrus_tail.py tail --lines 40

    # follow for a bounded time, printing as lines arrive
    python tools/papyrus_tail.py follow --seconds 20

    # only lines matching a pattern (case-insensitive regex)
    python tools/papyrus_tail.py tail --grep "charactergen|TES4"

    # a user log instead of the main VM log
    python tools/papyrus_tail.py tail --log User/TES4CharGen.0.log

    # errors only -- the usual first question
    python tools/papyrus_tail.py since --cursor 0 --errors
...
def resolve_log(name: str) -> Path:
    ...
    if not p.exists():
        raise SystemExit(
            f"no such log: {p}\n"
            f"(run `python tools/papyrus_tail.py list` to see what exists; "
...
def cmd_mark(args) -> int:
    ...
    if args.json:
        ...
        print(f"  python tools/papyrus_tail.py since --cursor {cursor}"

...
Why this exists: `tools/skyrim_disasm.py` can disassemble a known RVA, but many
...
Usage:
    ...
    python tools/skyrim_xref.py --exe <exe> --xref 0x1e64fc8 0x1e64fe0

    # find a Setting object by INI name, print its default value
    python tools/skyrim_xref.py --exe <exe> --setting fActivatePickRadius

    # find every INI setting whose name matches a substring
    python tools/skyrim_xref.py --exe <exe> --setting-search Pick

    # xref a setting by name in one step (finds object, xrefs its value field)
    python tools/skyrim_xref.py --exe <exe> --setting-xref fActivatePickRadius

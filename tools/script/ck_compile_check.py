...
Usage:
  ...
  python tools/script/ck_compile_check.py TES4_QF_FGC01Rats TES4_MG16MageScript

  # Check every script named in a scan file (one ScriptName per line):
  python tools/script/ck_compile_check.py --list suspects.txt

  # Check ALL converted scripts (slow):
  python tools/script/ck_compile_check.py --all

  # Check another plugin's scripts (default plugin is Oblivion.esm):
  python tools/script/ck_compile_check.py --plugin Morrowind_ob.esm --failed
"""
import argparse
import os
import re
